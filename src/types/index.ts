...
export interface OutlineInspection {
  ...
  preview: string
  text: string
  /** 无损索引预检数据；旧服务缺失这些字段时前端保持兼容。 */
  document_count?: number
  node_count?: number
  chunk_count?: number
  indexed_char_count?: number
  coverage_ratio?: number
  exact_source_covered?: boolean
  outline_index?: Record<string, unknown>
}
...
export type OutlineCompilationStatus =
  | 'queued'
  | 'running'
  | 'succeeded'
  | 'failed'
...
/** 持久化长大纲编译任务；刷新页面后可按 job_id 恢复观察。 */
export interface OutlineCompilationJob {
  job_id: string
  project_id: string
  work_session_id?: string | null
  status: OutlineCompilationStatus
  phase: string
  progress_percent: number
  terminal: boolean
  request: Partial<OutlineParams>
...
export interface ContinuousPolicy {
  ...
  /** 本次运行必须真实发生并持久化的 Critic/Continuity → Rewriter → 复审闭环数。 */
  minimum_rewrite_cycles: number
...
export type KnownSSEEventType =
  ...
  | 'chapter_completed'
  | 'quality_hold'
  | 'rewrite_contract_hold'
  | 'rewrite_contract_cycle_completed'
  | 'rewrite_contract_candidate_rejected'
  | 'budget_hold'
  | 'circuit_open'
  | 'retry_scheduled'
  | 'supervisor_failed'
...
export interface IntelligenceOutlineStats {
  document_id?: string
  revision?: number
...
export interface IntelligenceOverview {
  ...
  outline: {
    present: boolean
    stats: IntelligenceOutlineStats | null
    binding_count: number
    required_binding_count: number
    bound_node_count: number
    binding_status_counts: Record<string, number>
  }
  production: {
    scene_status_counts: Record<string, number>
    fidelity_assessment_count: number
    fidelity_passed_count: number
  }
  memory: {
...
export interface OutlineEvidenceNode {
  id: string
  ordinal: number
...
export interface OutlineEvidenceBinding {
  id: string
  chapter_no: number
  node_id: string
  source_path: string[] | string | null
...
export interface SceneFidelityEvidence {
  id: string
  score: number
  passed: boolean
  required_ids: string[]
  ...
  source_external_claims: unknown[]
}
...
export interface SceneEvidenceArtifact {
  ...
  fidelity: SceneFidelityEvidence | null
}

export interface ContextBundleEvidence {
  id: string
  purpose: string
  agent_role: string
  chapter_no: number | null
